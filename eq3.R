#' @name eq3
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ3
#'
                ...
                return = c("predict","calc","all")[1])
{
  ...
  if(!missing(data))
  {
    ...
    pars <- optim(par = pars,function(pars) sum(((data$A) -
              ((pars[["phi_I0"]]*data$PARi*pars[["Pgmax"]])/
                (((pars[["Pgmax"]]^2)+(pars[["phi_I0"]]^2)*(data$PARi^2))^0.5)-
                  pars[["Rd"]]))^2),control=list(fnscale=1))$par
    try({
      pars <- optim(par = pars,function(pars) sum(((data$A) -
            ((pars[["phi_I0"]]*data$PARi*pars[["Pgmax"]])/(((pars[["Pgmax"]]^2)+
              (pars[["phi_I0"]]^2)*(data$PARi^2))^0.5)-pars[["Rd"]]))^2),
                control=list(fnscale=1))$par
    },silent = TRUE)

    try({
      mod <- nls(control = nls.control(maxiter = 1000),A~(phi_I0*PARi*Pgmax)/
              (((Pgmax^2)+(phi_I0^2)*(PARi^2))^0.5)-Rd,data = data,start = pars)
      pars <- coef(mod)
    },silent = TRUE)
  }
  ...
  pred_fine <- (phi_I0*PARi_fine*Pgmax)/(((Pgmax^2)+
                                  (phi_I0^2)*(PARi_fine^2))^0.5)-Rd
  Isat_x <- ((((Pgmax-Rd)*x)+Rd)*Pgmax)/
    ((((Pgmax^2)-((((Pgmax-Rd)*x)+Rd)^2))^0.5)*phi_I0)
