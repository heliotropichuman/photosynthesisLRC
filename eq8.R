#' @name eq8
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ8
#'
#' @description Uses the nonlinear least squares exponential model
#'     equation 8 from Lobo et. al (2013) to transform measured photosynthetic
...
#'     Default values are Pgmax = 16.2, phi_I0 = .0597, and Rd = 1.3.
...
#'
#'     Webb, W. L., M. Newton, D. Starr 1974 Carbon dioxide exchange of
                ...
                return = c("predict","calc","all")[1])
{
  ...
  if(!missing(data))
  {
    ...
    pars <- optim(par = pars,function(pars) sum(((data$A) -
             ((pars[["Pgmax"]]*(1-exp(-pars[["phi_I0"]]*
                data$PARi/pars[["Pgmax"]]))) - pars[["Rd"]]))^2),
                   control=list(fnscale=1))$par

    try({
      mod <- nls(control = nls.control(maxiter = 1000),
              A~(Pgmax*(1-exp(-phi_I0*PARi/Pgmax)))-Rd,data = data,start = pars)
      pars <- coef(mod)
    },silent = TRUE)
  }
