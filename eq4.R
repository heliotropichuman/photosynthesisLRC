#' @name eq4
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ4
#'
#' @description Uses the nonlinear least squares hyperbolic tangent model
#'     equation 4 from Lobo et. al (2013) to transform measured photosynthetic
...
#'     Default values are Pgmax = 19.5, phi_I0 = 0.0493, and Rd = 1.8.
...
#'
#'     Abe, M., K. Yokota, A. Kurashima, M. Maegawa 2009 High water temperature
#'     tolerance in photosynthetic activity of *Zostera japonica* Ascherson and
#'     Graebner seedlings from Ago Bay, Mio Prefecture, central Japan. –
#'     Fish. Sci. 75: 1117-1123
#'
                ...
                return = c("predict","calc","all")[1])
{
  ...
  if(!missing(data))
  {
    ...
    pars <- optim(par = pars,function(pars) sum(((data$A) -
            ((pars[["Pgmax"]]*tanh(pars[["phi_I0"]]*data$PARi/pars[["Pgmax"]])-
    ...
    try({
      mod <- nls(control = nls.control(maxiter = 1000),
                 A~Pgmax*tanh(phi_I0*PARi/Pgmax)-Rd,data = data,start = pars)
      pars <- coef(mod)
    },silent = TRUE)
  }
