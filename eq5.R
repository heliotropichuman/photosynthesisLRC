#' @name eq5
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ5
#'
#' @description Uses the nonlinear least squares hyperbolic tangent model
#'     equation 5 from Lobo et. al (2013) to transform measured photosynthetic
...
#'     Default values are Pgmax = 15.5, Isat = 359.2, and Rd = 0.9.
...
#'     Pgmax, Isat, and Rd by minimizing the squared differences between
                ...
                return = c("predict","calc","all")[1])
{
  ...
  if(!missing(data))
  {
    ...
    pars <- optim(par = pars,function(pars) sum(((data$A) -
              (pars[["Pgmax"]]*tanh(data$PARi/pars[["Isat"]])-pars[["Rd"]]))^2),
                control=list(fnscale=1))$par
    try(pars <- optim(par = pars,function(pars) sum(((data$A) -
         (pars[["Pgmax"]]*tanh(data$PARi/pars[["Isat"]])-pars[["Rd"]]))^2),
           control=list(fnscale=1),method="BFGS")$par,silent=TRUE)
    try({
      mod <- nls(control = nls.control(maxiter = 1000),A~Pgmax*tanh(PARi/Isat)-
                   Rd,data = data,start = pars)
      pars <- coef(mod)
    },silent=TRUE)
  }
