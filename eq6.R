#' @name eq6
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ6
#'
#' @description Uses the nonlinear least squares non-rectangular hyperbola model
#'     equation 6 from Lobo et. al (2013) to transform measured photosynthetic
...
#'     Default values are Pgmax= 15.5, phi_I0= .0493, theta= .433, and Rd= .9
#'     These serve as initial starting parameters for the function to rapidly
#'     assess your data through an iterative process. The empirical coefficient
#'     theta falls between 0 and 1, and represents the various resistances
#'     faced by CO2 as it diffuses through the leaf mesophyll and is eventually
#'     bound by carboxylation enzymes. All of these values may be changed to
#'     fall within the minimum and maximum parameter values of your study
#'     system.
...
#'
#'     Prioul, J. L., P. Chartier 1977 Partitioning of transfer and
#'     carboxylation components of intracellular resistance to photosynthetic
#'     CO2 fixation: A critical analysis of the methods used. Ann. Bot.
#'     41: 789-800.
#'
...
#'
#' @export
eq6 <- function(pars = c(Pgmax = 15.5,phi_I0 = .0493,theta = .433,Rd = .9),
                data,
                PARi = c(0, 50, 100, 250, 500, 1000, 1500, 2000, 2500),
                return = c("predict","calc","all")[1])
{
  ...
  if(!missing(data))
  {
    ...
    pars <- optim(par = pars,function(pars) sum(((data$A) -
        ((((data$PARi*pars[["phi_I0"]]+pars[["Pgmax"]])-
         ((((pars[["phi_I0"]]*data$PARi+pars[["Pgmax"]])^2)-
          (4*pars[["phi_I0"]]*pars[["Pgmax"]]*pars[["theta"]]*data$PARi))^0.5))/
           (2*pars[["theta"]]))-exp(pars[["Rd"]])))^2),
            control=list(fnscale=1))$par
    try({ pars <- optim(par = pars,function(pars) sum(((data$A) -
        ((((data$PARi*pars[["phi_I0"]]+pars[["Pgmax"]])-
         ((((pars[["phi_I0"]]*data$PARi+pars[["Pgmax"]])^2)-
          (4*pars[["phi_I0"]]*pars[["Pgmax"]]*pars[["theta"]]*data$PARi))^0.5))/
            (2*pars[["theta"]]))-exp(pars[["Rd"]])))^2),control=list(fnscale=1),
              method="BFGS")$par},silent = TRUE)
    try({
      mod <- nls(control = nls.control(maxiter = 1000),A~(((PARi*phi_I0+Pgmax)-
                  ((((phi_I0*PARi+Pgmax)^2)-(4*phi_I0*Pgmax*theta*PARi))^0.5))/
                    (2*theta))-exp(Rd),data = data,start = pars)
      pars <- coef(mod)
    },silent = TRUE)
    pars[["Rd"]] <- exp(pars[["Rd"]])
  }
