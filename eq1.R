#' @name eq1
#' @title Calculate Photosynthetic Rates Using a Nonlinear Model EQ1
#'
#' @description Uses the nonlinear least squares Michaelis-Menton model
#'     equation 1 from Lobo et. al (2013) to transform measured photosynthetic
...
#'     Default values are Pgmax = 19.5, phi_I0 = 0.0899, and Rd = 1.8.
...
#'
#'     Baly, E.C.C. 1935 The kinetics of photosynthesis. Proc. R. Soc. Lond. B.
#'     117: 218-239.
#'
...
#'
#' @export
eq1 <- function(pars = c(Pgmax = 19.5,phi_I0 = .0899,Rd = 1.8),
                data,
                PARi = c(0, 50, 100, 250, 500, 1000, 1500, 2000, 2500),
                return = c("predict","calc","all")[1])
{
    ...
    if(!missing(data))
    {
      ...
      pars <- optim(par = pars,function(pars) sum(((data$A) -
               (((pars[["phi_I0"]]*data$PARi*pars[["Pgmax"]])/
                 (pars[["phi_I0"]]*data$PARi+pars[["Pgmax"]]))-pars[["Rd"]]))^2),
               control=list(fnscale=1))$par
      try({ pars <- optim(par = pars,function(pars) sum(((data$A) -
                     (((pars[["phi_I0"]]*data$PARi*pars[["Pgmax"]])/
                      (pars[["phi_I0"]]*data$PARi+pars[["Pgmax"]]))-
                        pars[["Rd"]]))^2),control=list(fnscale=1),method =
                       "BFGS")$par },silent = TRUE)

      try({
        mod <- nls(control = nls.control(maxiter = 1000),A~((phi_I0*PARi*Pgmax)/
                               (phi_I0*PARi+Pgmax))-Rd,data = data,start = pars)
        pars <- coef(mod)
      },silent = TRUE)
    }
